"""
Archipelago World definition for Pokemon Platinum Version
"""
...
from .client import PokemonPlatinumClient  # Unused, but required to register with BizHawkClient
from .data import (SpeciesData, MapData, EncounterTableData, LearnsetMove, TrainerPokemonData, StaticEncounterData,
                   TrainerData, data as platinum_data)
from .items import (ITEM_GROUPS, PokemonPlatinumItem, create_item_label_to_code_map, get_item_classification,
                    offset_item_value)
from .locations import (LOCATION_GROUPS, PokemonPlatinumLocation, create_location_label_to_id_map,
...
class PokemonPlatinumWebWorld(WebWorld):
    """
    Webhost info for Pokemon Platinum
    """
    theme = "ocean"
    setup_en = Tutorial(
...
class PokemonPlatinumWorld(World):
    ...
    badge_shuffle_info: Optional[List[Tuple[PokemonPlatinumLocation, PokemonPlatinumItem]]] = None
    hm_shuffle_info: Optional[List[Tuple[PokemonPlatinumLocation, PokemonPlatinumItem]]] = None
    ...
    def create_items(self) -> None:
        item_locations: List[PokemonPlatinumLocation] = [
            location
        ...
        if self.options.item_pool_type == ItemPoolType.option_shuffled:
            ...
            fill_item_candidates = platinum_data.items.values()
    ...
    def generate_basic(self) -> None:
        locations: List[PokemonPlatinumLocation] = self.multiworld.get_locations(self.player)
    ...
    def pre_fill(self) -> None:
        ...
        if self.options.badges == RandomizeBadges.option_shuffle:
            badge_locations: List[PokemonPlatinumLocation]
            badge_items: List[PokemonPlatinumItem]
        ...
        if self.options.hms == RandomizeHms.option_shuffle:
            hm_locations: List[PokemonPlatinumLocation]
            hm_items: List[PokemonPlatinumItem]
    ...
    def generate_output(self, output_directory: str) -> None:
        def randomize_abilities() -> None:
            ...
            ability_label_to_value = {ability.label.lower(): ability.ability_id for ability in platinum_data.abilities}
            ...
            ability_whitelist = [a.ability_id for a in platinum_data.abilities if a.ability_id not in ability_blacklist]
                    ...
                    if table is not None:
                        ...
                        for species_id in table.slots:
                            if species_id not in species_old_to_new_map:
                                original_species = platinum_data.species[species_id]
        ...
        def randomize_static_encounters() -> None:
            if self.options.static_encounters == RandomizeStaticEncounters.option_shuffle:
                shuffled_species = [encounter.species_id for encounter in platinum_data.static_encounters]
                self.random.shuffle(shuffled_species)

                self.modified_static_encounters = []
                for i, encounter in enumerate(platinum_data.static_encounters):
                ...
                for encounter in platinum_data.static_encounters:
        ...
        def randomize_opponent_parties() -> None:
            ...
            for trainer in self.modified_trainers:
                ...
                for pokemon in trainer.party.pokemon:
                    original_species = platinum_data.species[pokemon.species_id]
                    target_bst = sum(original_species.base_stats) if should_match_bst else None
                    target_type = self.random.choice(original_species.types) if should_match_type else None
        ...
        def randomize_starters() -> None:
            ...
            starter_bsts = (
                sum(platinum_data.species[platinum_data.starters[0]].base_stats) if match_bst else None,
                sum(platinum_data.species[platinum_data.starters[1]].base_stats) if match_bst else None,
                sum(platinum_data.species[platinum_data.starters[2]].base_stats) if match_bst else None
            )

            starter_types = (
                self.random.choice(platinum_data.species[platinum_data.starters[0]].types) if match_type else None,
                self.random.choice(platinum_data.species[platinum_data.starters[1]].types) if match_type else None,
                self.random.choice(platinum_data.species[platinum_data.starters[2]].types) if match_type else None
            )
            ...
            for i, starter in enumerate([new_starters[1], new_starters[2], new_starters[0]]):
                ...
                for trainer_name, starter_position, is_evolved in rival_teams[i]:
                    trainer_data = self.modified_trainers[platinum_data.constants[trainer_name]]
                    trainer_data.party.pokemon[starter_position].species_id = picked_evolution if is_evolved else starter.species_id

        self.modified_species = copy.deepcopy(platinum_data.species)
        self.modified_trainers = copy.deepcopy(platinum_data.trainers)
        self.modified_maps = copy.deepcopy(platinum_data.maps)
        self.modified_tmhm_moves = copy.deepcopy(platinum_data.tmhm_moves)
        self.modified_static_encounters = copy.deepcopy(platinum_data.static_encounters)
        self.modified_starters = copy.deepcopy(platinum_data.starters)
    ...
    def create_item_by_code(self, item_code: int) -> PokemonPlatinumItem:
